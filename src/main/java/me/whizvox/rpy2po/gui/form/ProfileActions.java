...
public class ProfileActions extends JFrame {
  ...
  private JButton buttonCreateTranslations;
  ...
  public ProfileActions(Profile profile) {
    ...
    buttonGenTemplate.addMouseListener(new UpdateOnMouseEnteredListener(textAreaFeedback, "Read the project's " + profile.getPrimaryLanguage() + " Ren'Py translation (.rpy) files to generate a template (.pot) file."));
    buttonCreateTranslations.addActionListener(e -> generateCatalogs());
    buttonCreateTranslations.addMouseListener(new UpdateOnMouseEnteredListener(textAreaFeedback, "Copy the template (.pot) file to generate all missing language (.po) files."));
    buttonImport.addActionListener(e -> importFiles());
    buttonImport.addMouseListener(new UpdateOnMouseEnteredListener(textAreaFeedback, "Import project's translation (.rpy) files and convert them into language (.po) files."));
    buttonUpdate.addActionListener(e -> updateFiles());
    buttonUpdate.addMouseListener(new UpdateOnMouseEnteredListener(textAreaFeedback, "Update the language (.po) files to match the template (.pot) file. Should use whenever a change to the game script has been made."));
    buttonExport.addActionListener(e -> exportFiles());
    buttonExport.addMouseListener(new UpdateOnMouseEnteredListener(textAreaFeedback, "Convert all language (.po) files into Ren'Py translation (.rpy) files."));
    textFieldOutLangs.getDocument().addDocumentListener((DocumentChangedListener) e -> {
      buttonSaveSettings.setEnabled(true);
    });
    textFieldOutLangs.addMouseListener(new UpdateOnMouseEnteredListener(textAreaFeedback, "All languages that your project will be translated into. Example:\nde, es, ru, fr, zh_hans"));
    ...
    buttonNames.addMouseListener(new UpdateOnMouseEnteredListener(textAreaFeedback, "Character names that appear when generating language (.po) files to assist translators."));
    buttonFiles.addActionListener(e -> RPY2PO.inst().setFrame(() -> new IncludeFiles(profile), "Set Files", null));
    buttonFiles.addMouseListener(new UpdateOnMouseEnteredListener(textAreaFeedback, "Specify which Ren'Py (.rpy) files to include and exclude in the template (.pot) and language (.po) files."));
  }

  private void enableActions(boolean enable, boolean includeSettings) {
    buttonGenTemplate.setEnabled(enable);
    buttonCreateTranslations.setEnabled(enable);
...
// DO NOT EDIT OR ADD ANY CODE HERE!
  ...
  private void $$$setupUI$$$() {
    ...
    buttonCreateTranslations = new JButton();
    buttonCreateTranslations.setText("Create PO Translations");
    panel1.add(buttonCreateTranslations, new GridConstraints(3, 0, 1, 1, GridConstraints.ANCHOR_CENTER, GridConstraints.FILL_HORIZONTAL, GridConstraints.SIZEPOLICY_CAN_SHRINK | GridConstraints.SIZEPOLICY_CAN_GROW, GridConstraints.SIZEPOLICY_FIXED, null, new Dimension(-1, 40), null, 0, false));

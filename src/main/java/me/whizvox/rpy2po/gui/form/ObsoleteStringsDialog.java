...
public class ObsoleteStringsDialog extends JDialog {

  private JPanel contentPane;
  private JList<String> listStrings;
  private JLabel labelReference;
  private JTextArea textAreaString;
  private JButton buttonObsolete;
  private JButton buttonBack;
  private JButton buttonAllObsolete;
  private JLabel labelContext;
  private JLabel labelComment;
  private JCheckBox checkBoxShow;

  private List<Message> strings;

  public ObsoleteStringsDialog(ResolveTranslationProblems owner) {
    super(owner, true);
    setTitle("Found Unresolved Strings");
    strings = new ArrayList<>(owner.getUnaccountedMessages(false));
    ...
    checkBoxShow.addActionListener(e -> {
      strings = new ArrayList<>(owner.getUnaccountedMessages(checkBoxShow.isSelected()));
      model.clear();
      model.addAll(strings.stream().map(Message::getMsgId).toList());
      if (checkBoxShow.isSelected()) {
        buttonObsolete.setText($$$getMessageFromBundle$$$("strings", "button.unmarkObsolete"));
      } else {
        buttonObsolete.setText($$$getMessageFromBundle$$$("strings", "button.markObsolete"));
      }
    });
    buttonBack.addActionListener(e -> hide());
    buttonObsolete.addActionListener(e -> {
      int row = listStrings.getLeadSelectionIndex();
      if (row >= 0 && row < strings.size()) {
        owner.markObsolete(strings.get(row), !checkBoxShow.isSelected());
        strings.remove(row);
        model.remove(row);
        int i = Math.min(row, model.getSize());
        listStrings.getSelectionModel().addSelectionInterval(i, i);
      }
    });
    buttonAllObsolete.addActionListener(e -> strings.forEach(msg -> owner.markObsolete(msg, !checkBoxShow.isSelected())));
...
// DO NOT EDIT OR ADD ANY CODE HERE!
  ...
  private void $$$setupUI$$$() {
    ...
    panel2.add(spacer1, new GridConstraints(9, 0, 1, 1, GridConstraints.ANCHOR_CENTER, GridConstraints.FILL_VERTICAL, 1, GridConstraints.SIZEPOLICY_WANT_GROW, null, null, null, 0, false));
    ...
    buttonObsolete = new JButton();
    this.$$$loadButtonText$$$(buttonObsolete, this.$$$getMessageFromBundle$$$("strings", "button.markObsolete"));
    panel2.add(buttonObsolete, new GridConstraints(8, 0, 1, 1, GridConstraints.ANCHOR_CENTER, GridConstraints.FILL_NONE, 1, GridConstraints.SIZEPOLICY_FIXED, null, null, null, 0, false));
    ...
    checkBoxShow = new JCheckBox();
    checkBoxShow.setText("Show Obsolete Strings");
    panel1.add(checkBoxShow, new GridConstraints(1, 0, 1, 1, GridConstraints.ANCHOR_NORTHEAST, GridConstraints.FILL_NONE, GridConstraints.SIZEPOLICY_CAN_SHRINK | GridConstraints.SIZEPOLICY_CAN_GROW, GridConstraints.SIZEPOLICY_FIXED, null, null, null, 0, false));
    ...
    buttonAllObsolete = new JButton();
    buttonAllObsolete.setText("Mark All Obsolete");
    panel3.add(buttonAllObsolete, new GridConstraints(0, 2, 1, 1, GridConstraints.ANCHOR_CENTER, GridConstraints.FILL_HORIZONTAL, GridConstraints.SIZEPOLICY_CAN_SHRINK | GridConstraints.SIZEPOLICY_CAN_GROW, GridConstraints.SIZEPOLICY_FIXED, null, null, null, 0, false));
  }
